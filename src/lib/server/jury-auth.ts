...
function getSecret() {
    const secret = env.JURY_SESSION_SECRET ?? env.JWT_SECRET ?? env.BETTER_AUTH_SECRET;
    if (!secret) {
        if (dev) {
...
export function setJurySessionCookie(cookies: import("@sveltejs/kit").Cookies, jurorId: string) {
...
export function clearJurySessionCookie(cookies: import("@sveltejs/kit").Cookies) {
