...
export const actions: Actions = {
    leaveTeam: async (event) => {
        ...
        if (team.captainId === user.id) {
            await prisma.juryAssignment.deleteMany({ where: { submission: { teamId } } });
            await prisma.submission.deleteMany({ where: { teamId } });
            await prisma.teamMember.deleteMany({ where: { teamId } });
            await prisma.team.delete({ where: { id: teamId } });
        } else {
            await prisma.teamMember.deleteMany({ where: { teamId, email: user.email } });
        }
    ...
    inviteMember: async (event) => {
        ...
        const existing = await prisma.teamMember.findFirst({ where: { teamId, email } });
