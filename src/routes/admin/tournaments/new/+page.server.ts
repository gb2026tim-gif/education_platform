    ...
    try {
      await prisma.tournament.create({
        data: {
          ...
          regStart: data.get("regStart")
            ? new Date(data.get("regStart") as string)
            : null,
          regEnd: data.get("regEnd")
            ? new Date(data.get("regEnd") as string)
            : null,
          startAt: data.get("startAt")
            ? new Date(data.get("startAt") as string)
            : null,
          taskDeadline: data.get("taskDeadline")
            ? new Date(data.get("taskDeadline") as string)
            : null,
