  ...
  generate: async ({ request, locals }) => {
    ...
    try {
      ...
      if (entityType === "tournament") {
        ...
        const memberEmails = teams.flatMap((t) =>
        ...
        userIds = [
          ...new Set([...captainIds, ...memberUsers.map((u) => u.id)]),
        ];
      } else {
      ...
      if (userIds.length === 0) {
        return fail(400, {
          error: "Немає учасників для генерації сертифікатів",
        });
      }
      ...
      return {
        success: true,
        message: `Видано ${created} нових сертифікатів (${userIds.length - created} вже мали)`,
      };
    } catch (e: unknown) {
      return fail(500, {
  ...
  assignTemplate: async ({ request, locals }) => {
    ...
    if (entityType === "tournament") {
      await prisma.tournament.update({
        where: { id: entityId },
        data: { certTemplateId: templateId },
      });
    } else {
